...
function convert_video_uid0014() {
    [[ -z "$1" ]] && return 1

    local input_file="$1"
    local output_file="${2:-"${input_file%.*}.uid0014.avi"}"
    ...
    local ffmpeg_args=(
        ...
        -f avi
        -c:v libx264
        -x264-params "mbtree=0:rc-lookahead=0:aq-mode=2:ipratio=2:psy=0:me=tesa:subme=11:trellis=2:weightp=2"
        -profile:v baseline
        -filter:v
        "
...
[[ "${BASH_SOURCE[0]}" == "$0" ]] && convert_video_uid0014 "$@"
