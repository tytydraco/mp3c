...
function convert_video_uid0009() {
    ...
    local ffmpeg_args=(
        ...
        -f amv                                                                                                                              # AMV container.
        -c:v amv                                                                                                                            # AMV codec.
        -filter:v "scale=$size:force_original_aspect_ratio=increase,crop=$size:(iw-ow)/2:(ih-oh)/2,transpose=cclock:passthrough=landscape"  # Contain within size, preserve aspect ratio, crop, pre-rotate clockwise (landscape bypass).
